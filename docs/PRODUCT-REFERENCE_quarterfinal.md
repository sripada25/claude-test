...
| **Frontend** | M03 (16) + M04 (6) + M05 (10) = 32 components |
...
| **Blocking** | ~~🔴 L038 (copy-only vs Pro send) gates 3 of 9 tasks. Evidence is one-sided toward copy-only — see the ledger record~~ — resolved as of 2026-09-12: L038's Pro send path is both decided and now implemented (**M08-R1**/**M08-R2-PRE**/**M08-R2**). No longer blocking |
...
| **Status** | Checkout flow substantially designed. 6 new backend tasks (F6-1 to F6-6) in `TASKS_quarterfinal.md`. Still last in the build order — the `subscriptions` stub in F1 continues to unblock everything downstream. **M09-1** (Settings page shell — Account + read-only Plan display + real delete-account) merged via PR #308 (2026-09-14), ahead of F6 itself; Upgrade/checkout stays disabled until F6-1 through F6-3 exist. **M09-2** (`PATCH /api/account` for `reminder_emails_enabled`) merged via PR #310, **M09-3** (Notifications section, wired to M09-2) merged via PR #312 (both 2026-09-14) — "Follow-up reminders by email" is fully real end to end. "Weekly pipeline summary" (the mockup's other toggle) remains out of scope — no digest feature exists at all, shown disabled. **M09-4** (`POST /api/auth/change-password`, backend) merged via PR #314; **M09-5** (wired the frontend, and fixed a real bug its own live verification found in M09-4 — the route trusted `x-user-id`, which `proxy.ts` never sets under its public `/api/auth/` prefix, so every real request 401'd; now resolves its own session from the cookie like `change-email` already did) merged via PR #316 (both 2026-09-14) — in-app change/set-password is fully real end to end, verified by actually signing in with the new password. **M09-6** (`GET /api/account/export`, backend — closes the data-export gap `PRIVACY.md` names as a DPDP Act 2023 requirement) merged via PR #318 (2026-09-14); ~~`DataPrivacySection.tsx`'s "Export all my data" still shows "Coming soon" until a frontend task wires it up~~ **M09-7** (wired "Export all my data" to a real `<a href="/api/account/export">` link — no fetch/blob needed since M09-6 already sets `Content-Disposition: attachment`) merged via PR #320 (2026-09-14) — export is fully real end to end, live-verified against a real signed-in session. Settings screen is now fully real except Upgrade/checkout (blocked on F6) and call-transcript retention (mockup itself says "TBD") |
...
| — | ~~**Pro send-path design** — L038 is *decided* (both tiers), but confirmation/sent/failure states don't exist~~ — done: **M08-R2**, merged via PR #303 (2026-09-12). 08b/08c/08d states built, real send implemented | ~~3 of F4's 9 tasks~~ none, resolved |
| — | ~~**`contact_email` verification** before use as Reply-To — now required~~ — done: **M08-R2-PRE**, merged via PR #301 (2026-09-12). `profiles.contact_email_verified_at` gates Reply-To use, consumed by **M08-R2** | F4 send path |
