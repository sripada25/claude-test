...
describe("016_generation_jobs_quota_mechanism migration (real Postgres)", () => {
  ...
  it("adds quota_mechanism as a nullable text column", async () => {
    await migrate.up();
    ...
    const result = await pool.query<{ udt_name: string; is_nullable: string }>(
      `SELECT udt_name, is_nullable FROM information_schema.columns
       WHERE table_name = 'generation_jobs' AND column_name = 'quota_mechanism'`,
    );

    expect(result.rows).toEqual([{ udt_name: "text", is_nullable: "YES" }]);
  });

  it("leaves existing rows with a NULL quota_mechanism", async () => {
    await migrate.up();
    ...
    const jobResult = await pool.query<{ quota_mechanism: string | null }>(
      `INSERT INTO generation_jobs (user_id, application_id, type, prompt_inputs)
       VALUES ($1, $2, 'cover_letter', '{}') RETURNING quota_mechanism`,
      [userResult.rows[0].id, applicationResult.rows[0].id],
    );

    expect(jobResult.rows[0].quota_mechanism).toBeNull();
  });
  ...
  it("is fully reversible: down() drops the column, and up() re-applies cleanly", async () => {
    ...
    // keep reverting until 016 itself is undone - resilient to any
    // migration added after this one.
    while (await columnExists("generation_jobs", "quota_mechanism")) {
      await migrate.down();
    }
    expect(await columnExists("generation_jobs", "quota_mechanism")).toBe(false);

    await expect(migrate.up()).resolves.not.toThrow();
    expect(await columnExists("generation_jobs", "quota_mechanism")).toBe(true);
  });
});
